...
Skill *init_evasive_maneuver(){
    Skill *evasive_maneuver = (Skill*)malloc(sizeof(Skill));
    strcpy(evasive_maneuver->name, "Evasive Maneuver");
    strcpy(evasive_maneuver->description, "Captain Blackbeard quickly dodges, making him harder to hit for a turn.");
    evasive_maneuver->type = 0; // temporal modifier
    evasive_maneuver->modifier[0] = NULL; // HP is unchanged
    evasive_maneuver->modifier[1] = NULL; // ATK is unchanged
    evasive_maneuver->modifier[2] = 2; // increases evasion
    evasive_maneuver->duration_turn = 1; // modifies DEF points for only that turn
    return evasive_maneuver;
}
...
Skill *init_time_strike(){
    Skill *time_strike = (Skill*)malloc(sizeof(Skill));
    strcpy(time_strike->name, "Time Strike");
    strcpy(time_strike->description, "Strikes with time power, dealing damage and weakening enemies' attacks.");
    time_strike->type = 1; // direct attack
    time_strike->modifier[0] = 1.5; // moderate damage
    time_strike->modifier[1] = 0.8; // reduces opponent's attack power
    time_strike->modifier[2] = NULL; // DEF is unchanged
    time_strike->duration_turn = 1; // modifies ATK points for only that turn
    return time_strike;
}
